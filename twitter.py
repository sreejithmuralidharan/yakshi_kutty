...
search_term = "*"
while True:
    for tweet in tweepy.Cursor(twitter.search, q=search_term, lang=search_language, result_type = 'recent').items():
        now = datetime.today().now()
        if now.hour>= bed_time:
            post_data = {
                'status': 'Sleeping',
                'message': 'Goodnight, See you after 6 hours',
                'timestamp': datetime.today().now()
            }
            mongo_db.insert_one(post_data)
            print('time for bed, speeping for 6 hours')
